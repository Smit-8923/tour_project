...
include 'config.php';
$id = $_GET['id'];
$sql = "SELECT * FROM destination_table WHERE d_id = ?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  $d_name = $_POST['d_name'];
  $update = "UPDATE destination_table SET d_name = ? WHERE d_id = ?";
  $stmt = $conn->prepare($update);
  $stmt->bind_param("si", $d_name, $id);
  if ($stmt->execute()) {
    header("Location: manage-destination.php?msg=Destination updated successfully");
    exit;
  } else {
...
<body>
  ...
  <div class="container mt-5">
    ...
    <form method="POST">
      <div class="mb-3">
        <label for="d_name" class="form-label">Destination Name</label>
        <input type="text" name="d_name" value="<?= $row['d_name']; ?>" class="form-control" required>
      </div>
      <button type="submit" class="btn btn-success">Update</button>
      <a href="manage-destination.php" class="btn btn-secondary">Back</a>
    </form>
  </div>
