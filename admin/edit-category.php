...
$c_id = $_GET['edit'];
$result = $conn->query("SELECT * FROM category_table WHERE c_id = $c_id");
$row = $result->fetch_assoc();

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $c_name = $_POST['c_name'];
    $conn->query("UPDATE category_table SET c_name='$c_name' WHERE c_id=$c_id");
...
<div class="container mt-5">
    ...
    <form method="POST">
        <div class="mb-3">
            ...
            <input type="text" class="form-control" name="c_name" value="<?= htmlspecialchars($row['c_name']) ?>" required>
        </div>
